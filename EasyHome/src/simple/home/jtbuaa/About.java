...
public class About extends Activity {
    ...
    String runCmd(String cmd, String para) {// performance of runCmd is very
                                            // low, may cause black screen. do
                                            // not use it AFAC
        String line = "";
        try {
            String[] cmds = {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        btnShareHome.setText(getString(R.string.app_name) + " " + util.getVersion(getBaseContext())
         ...
         * @Override public void onClick(View v) { Intent intentShareDesktop =
         * new Intent("simpleHome.action.SHARE_DESKTOP");
         * sendBroadcast(intentShareDesktop);//need get screen of home, so send
         * intent to home } });
         */
            ...
            public void onClick(View v) {
                ...
                if (file.equals("")) {
                    ...
                    try {
                        ...
                        BitmapDrawable bd = (BitmapDrawable) WallpaperManager.getInstance(
                                getBaseContext()).getDrawable();
                ...
                util.startActivity(Intent.createChooser(intent, getString(R.string.sharemode)),
                        true, getBaseContext());
            }
        });
        ...
        rotateMode.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup arg0, int arg1) {
                editor.putInt("rotate_mode",
                        rotateMode.indexOfChild(findViewById(rotateMode.getCheckedRadioButtonId())));
            }
        });

        /*
         * systemApp = (RadioGroup) findViewById(R.id.system_mode); boolean
         * isGrid = perferences.getBoolean("system", true); if (isGrid) tmpMode
         * = 1; else tmpMode = 2; ((RadioButton)
         * systemApp.getChildAt(tmpMode)).setChecked(true);
         ...
         * int grid =
         * systemApp.indexOfChild(findViewById(systemApp.getCheckedRadioButtonId
         * ())); if (grid == 1) editor.putBoolean("system", true); else
         * editor.putBoolean("system", false); } }); userApp = (RadioGroup)
         * findViewById(R.id.user_mode); isGrid = perferences.getBoolean("user",
         * false); if (isGrid) tmpMode = 1; else tmpMode = 2; ((RadioButton)
         * userApp.getChildAt(tmpMode)).setChecked(true);
         * userApp.setOnCheckedChangeListener(new OnCheckedChangeListener() {
         * @Override public void onCheckedChanged(RadioGroup arg0, int arg1) {
         * int grid =
         * userApp.indexOfChild(findViewById(userApp.getCheckedRadioButtonId
         * ())); if (grid == 1) editor.putBoolean("user", true); else
    ...
    private void removeDefaultHome() {// refer to
                                      // http://www.dotblogs.com.tw/neil/archive/2011/08/12/33058.aspx
        ...
        // we don't start myself again for we use system chooser to select home,
        // and won't go back to myself
        // Intent intent = new Intent();
        // intent.setClassName(getApplicationContext(),
        // SelectHome.class.getName());
    ...
    private void selectOphoneHome() {
        ...
        Collections.sort(mHomeList, new ResolveInfo.DisplayNameComparator(mPM));// sort
                                                                                // by
                                                                                // name
        ...
        if (configuredHome == null) {// first run after install
            ...
            for (int i = 0; i < mHomeList.size(); i++) {
                ...
                if (!oldName.equals(mPackageName))
                    ...
                    myName = mHomeList.get(i).activityInfo.name;
            }
            ...
            new AlertDialog.Builder(this)
                    .setTitle(R.string.menu_choose_home)
                    .setSingleChoiceItems(mTitle, currentHomeIndex,
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    if (!mPackageName.equals(oldName)) {// try
                                                                        // to
                                                                        // close
                                                                        // the
                                                                        // old
                                    ...
                                    } else {// if I'm the old home, not stop me
                                            // immediately, otherwise the new
                                            // home may not launch
                                        Intent intentNewhome = new Intent(
                                                "simpleHome.action.HOME_CHANGED");
                                    ...
                                    intent.addCategory(Intent.CATEGORY_HOME);
                                    intent.setClassName(
                                            mHomeList.get(which).activityInfo.packageName,
                                            mHomeList.get(which).activityInfo.name);
                                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                                            | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
                                    startActivity(intent);
