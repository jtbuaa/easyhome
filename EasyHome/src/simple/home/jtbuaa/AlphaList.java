...
public abstract class AlphaList<T> {
    ...
    int mColumns = 10000;// just a big decimal to ensure not adjust gravity in
                         // grid mode
    int mSelected = -1;
    ...
    static int whiteColor = 0xFFFFFFFF, grayColor = 0xDDDDDDDD, redColor = 0xFFFF7777,
            brownColor = 0xFFF8BF00;
    ...
    AlphaList(Context context, PackageManager pmgr, HashMap<String, Object> packageSize,
            boolean isGrid, boolean largeScreen) {
        mContext = context;
        pm = pmgr;
        ...
        if (mIsGrid)
            appGridAdapter = new AppGridAdapter(mContext, mApps);
        else
            appListAdapter = new AppListAdapter(mContext, mApps);
        ...
        // init UI
        if (mIsGrid)
            view = (RelativeLayout) ((Activity) mContext).getLayoutInflater().inflate(
                    R.layout.grid_apps, null);
        else
            view = (RelativeLayout) ((Activity) mContext).getLayoutInflater().inflate(
                    R.layout.apps, null);
        ...
        AppList.setOnScrollListener(new OnScrollListener() {
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount,
                    int totalItemCount) {
                if ((mApps.size() > firstVisibleItem) && (!DuringSelection)) {// revert
                                                                              // the
                                                                              // focus
                                                                              // of
                                                                              // alpha
                    ...
                    if (pos != mSelected) {
                        ...
                        if (tv != null)
                            tv.setBackgroundResource(R.drawable.circle);// it
                                                                        // may
                                                                        // be
                                                                        // circle_selected
                                                                        // when
                        ...
                        if (tv != null)
                            tv.requestFocus();// this will change its background
                                              // color
            ...
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                DuringSelection = false;// the scrollState will not change when
                                        // setSelection(), but will change
                                        // during scroll manually. so we turn
                                        // off the flag here.
                // if (scrollState ==
                // AbsListView.OnScrollListener.SCROLL_STATE_IDLE) scrolling =
                // false;
    ...
    void setAdapter() {
        ...
        if (mIsGrid)
            AppList.setAdapter(appGridAdapter);
        else
    ...
    void add(T ri, boolean sort, boolean updateAlpha) {
        if (mIsGrid) {
            ...
            Collections.sort(appGridAdapter.localApplist, new myComparator());// sort
                                                                              // by
                                                                              // name
        } else {
            appListAdapter.add(ri);
            Collections.sort(appListAdapter.localApplist, new myComparator());// sort
                                                                              // by
                                                                              // name
        }
    ...
    void setColumns() {// set column number of alpha grid
        ...
        } else {// keep not more than 3 lines in list mode, each line should
                // have almost the same count of alpha
            mColumns = MaxCount;
            if (alphaAdapter.getCount() < MaxCount)
                mColumns = alphaAdapter.getCount();
            else if (alphaAdapter.getCount() < MaxCount * 2)
                mColumns = (int) (alphaAdapter.getCount() / 2.0 + 0.5);
    ...
    static {
        try {
            forceStopPackage = ActivityManager.class.getDeclaredMethod("forceStopPackage",
                    String.class);
        } catch (Exception e) {
        }
    }
    ...
    private class AppListAdapter extends ArrayAdapter<T> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null)
                convertView = ((Activity) mContext).getLayoutInflater().inflate(R.layout.app_list,
                        parent, false);

            final TextView textView1 = (TextView) convertView.findViewById(R.id.appname);

            if (getLabel(info).equals(textView1.getText()) && (DuringSelection))// don't
                                                                                // update
                                                                                // the
                                                                                // view
                                                                                // here
            ...
            if (!getLabel(info).equals(textView1.getText())) {// only reset the
                                                              // appname,
                                                              // version, icon
                                                              // when needed
                ...
                btnIcon.setOnTouchListener(new OnTouchListener() {
                    @Override
                    public boolean onTouch(View v, MotionEvent event) {// kill
                                                                       // process
                                                                       // when
                                                                       // click
                        ...
                        if (!pn.equals("simple.home.jtbuaa")) {
                            ...
                            // but we need to know when will it restart by
                            // itself?
                            textView1.setTextColor(whiteColor);// set color back
                                                               // after kill it.
                        }
                        return false;
                    }
                ...
                lapp.setOnTouchListener(new OnTouchListener() {
                    ...
                    public boolean onTouch(View v, MotionEvent event) {
                        long pressTime = event.getEventTime() - event.getDownTime();// use
                                                                                    // this
                                                                                    // to
                                                                                    // avoid
                                                                                    // long
                                                                                    // click
                        if ((pressTime < ViewConfiguration.getLongPressTimeout())
                                && (event.getAction() == MotionEvent.ACTION_UP)) {// start
                                                                                  // app
                                                                                  // when
                                                                                  // click
                            if (start(info))// start success
                                textView1.setTextColor(redColor);// red for
                                                                 // running apk
                            return true;
                        } else
                            return false;
                ...
                try {
                    ...
                    if ((version == null) || (version.trim().equals("")))
                        version = String
                                .valueOf(pm.getPackageInfo(getPackageName(info), 0).versionCode);
                ...
                if ((getFlag(info) & ApplicationInfo.FLAG_DEBUGGABLE) == ApplicationInfo.FLAG_DEBUGGABLE) {
                    textView3.setTextColor(brownColor);// brown for debuggable
                                                       // apk
                    source += " (debuggable)";
                } else
                    textView3.setTextColor(grayColor);// gray for normal
                textView3.setText(source);
            }

            textView1.setTextColor(whiteColor);// default color
            if (!DuringSelection) {// running state should be updated when not
                                   // busy, for it is time consuming
                final ActivityManager am = (ActivityManager) mContext
                        .getSystemService(Context.ACTIVITY_SERVICE);
                List<RunningAppProcessInfo> appList = am.getRunningAppProcesses();
                for (int i = 0; i < appList.size(); i++) {// a bottle neck
                    if (DuringSelection)
                        break;// cancel current task if enter scroll mode will
                              // raise performance significantly
    ...
    private class AppGridAdapter extends ArrayAdapter<T> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null)
                convertView = ((Activity) mContext).getLayoutInflater().inflate(R.layout.icon_list,
                        parent, false);
                ...
                btnIcon.setOnTouchListener(new OnTouchListener() {
                    ...
                    public boolean onTouch(View v, MotionEvent event) {// kill
                        ...
                        long pressTime = event.getEventTime() - event.getDownTime();// use
                                                                                    ...
                                                                                    // click
                        if ((pressTime > 0)
                                && (pressTime < ViewConfiguration.getLongPressTimeout())
                                && (event.getAction() == MotionEvent.ACTION_UP)) {// start
                                                                                  // app
                                                                                  // when
                                                                                  // click
        ...
        if (m_deleteDialog == null) {
            m_deleteDialog = new AlertDialog.Builder(mContext)
                    ...
                    .setPositiveButton(mContext.getString(R.string.delete),
                            new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {// rm
                                                                                        // system
                                                                                        // app
                                    ...
                                    Uri uri = Uri.fromParts("package", getPackageName(appToDel),
                                            null);
                                    Intent intent = new Intent(Intent.ACTION_DELETE, uri);
                                    util.startActivity(intent, true, mContext);// this
                                                                               // will
                                                                               // launch
                                                                               // package
                                                                               // installer.
    ...
    private class AlphaAdapter extends ArrayAdapter<String> {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            if (alphaList.size() > mColumns) {// only tune it if more than one
                                              // line
            ...
            btn.setOnTouchListener(new OnTouchListener() {
                @Override
                public boolean onTouch(View v, MotionEvent event) {// find app
                                                                   // when click
                    String tmp = localList.get(position);
                    DuringSelection = true;
                    v.requestFocusFromTouch();// this will make app list get
                                              // focus, very strange
                    TextView tv = (TextView) AlphaGrid.getChildAt(mSelected);// restore
                                                                             // the
                                                                             // background
                    if (tv != null)
                    ...
                    v.setBackgroundResource(R.drawable.circle_selected);// only
                                                                        // set
                                                                        // the
                                                                        // background
                                                                        // of
