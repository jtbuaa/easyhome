...
public class CrashControl extends Activity {
    ...
    private void init(final Intent intent) {
        ...
        if (errorMsg == null)
            retry = 1;
        else if (errorMsg.contains("WebViewDatabase"))
            retry = 1;// should clear the database and retry if SQLite*
                      // exception
        ...
        if (retry == 2) {
            ...
            btnRetry.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    Intent intent = new Intent(Intent.ACTION_VIEW, Uri
                            .parse("market://details?id=easy.browser.com"));
                    util.startActivity(intent, true, getBaseContext());
                    finish();
                }
            });
        } else if (retry == 1) {// clear database and retry to restart the
                                // browser
            ...
            btnRetry.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View arg0) {
                    ...
                    try {
                        Method setPackage = Intent.class.getDeclaredMethod("setPackage",
                                String.class);
            ...
            btnRetry.setText(getString(R.string.feed_back));
            btnRetry.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View arg0) {
                    ...
                    intent.setData(Uri
                            .fromParts("mailto", getString(R.string.browser_author), null));
                    intent.putExtra(Intent.EXTRA_TEXT, getString(R.string.comments)
                            + "\n\n\n\n\n====================\n" + sb.toString());
                    intent.putExtra(Intent.EXTRA_SUBJECT, getPackageName()
                            + getString(R.string.error_log));
                    if (!util.startActivity(intent, false, CrashControl.this)) {
                        // send mail by webpage if fail to send through mail
                        // client
                        Uri data = Uri
                                .parse("https://mail.google.com/mail/?ui=2&view=cm&fs=1&tf=1&su="
                                        + getString(R.string.error_log) + "&to="
                                        + getString(R.string.browser_author) + "&body="
